...
class GameTracker {
    ...
    renderTable(rows, hiddenRows, hiddenColumns) {
        ...
        // Create header row - include all columns, even empty ones
        for (let i = 0; i < maxColumns; i++) {
            ...
            // Use empty string if header doesn't exist for this column
        ...
        for (let i = 1; i < rows.length; i++) {
            ...
            if (!rows[i] || rows[i].every(cell => !cell || cell.trim() === '')) {
                const spacerTd = document.createElement('td');
                spacerTd.colSpan = maxColumns;
                spacerTd.style.height = '1.5rem'; // Adjust height as needed
